...
RUN echo "data fetch ${DATA_CACHEBUST}" \
 && python -c "import urllib.request; urllib.request.urlretrieve('${NAVAIDS_DB_URL}', '/build/navaids.csv')"
...
COPY --from=builder /build/navaids.csv /app/app/navaids.csv
